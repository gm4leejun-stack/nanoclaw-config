...
period_rows = [
    ('Today', M(gi+go), f'${usd(gi,go):.2f}', f'{gq}x'),
    ('Week',  M(wi+wo), f'${usd(wi,wo):.2f}', f'{wq}x'),
    ('30d',   M(mi+mo), f'${usd(mi,mo):.2f}', f'{mq}x'),
]
io_rows = [
    ('Input',  M(mi), f'${ic:.2f}', f'{ip:.0f}%'),
    ('Output', M(mo), f'${oc:.2f}', f'{100-ip:.0f}%'),
]
group_rows = []
